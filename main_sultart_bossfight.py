...
class Sol(pygame.sprite.Sprite):
    def __init__(self,x,y,w,h, *groups):
        ...
        self.image = pygame.surface.Surface((w,h), pygame.SRCALPHA)
...
class DieZone(pygame.sprite.Sprite):
    def __init__(self, *groups):
        super().__init__(*groups)
        self.size = [0, 0]
        self.x = 0
        self.y = 0
        self.image = pygame.surface.Surface((self.size[0], self.size[1]), pygame.SRCALPHA)
        self.rect = self.image.get_rect()
        self.rect.topleft = (self.x, self.y)

    """def draw(self, screen):
        pygame.draw.circle(screen,self.image,self.rect)"""
...
def main(ecran): 
    ...
    background, bg_image = get_background(bckg.get_imgCourante()) # Récupère les tiles et l'image de fond

    """ Crée un joueur, un train et un sol """
    joueur = Joueur(200, 200) # Crée un joueur
    sol = Sol(0,HAUTEUR-45, 1920, 20) # Crée un sol
